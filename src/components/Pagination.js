    ...
    <div>
        <div className='clearfix'>
            ...
            { 
                ...
                return (
                    <li key = {index} className="`${currentButton === page ? 'page-item active' : 'page-item' }`"> <a href='#' onClick={() => setCurrentButton(page)} className='page-link'>{page}</a></li>
                );
                }) 
            }

                <li className=" `${ currentButton === numOfPages.length ? 'page-item disabled ? 'page-item'}`"> <a href='#'
                onClick={() => setCurrentButton((next) => next === numOfPages.length ? next : next + 1)}>Next</a></li>                        
